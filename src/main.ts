...
<main class="experience" id="experience">
  ...
  <div class="flight-hud flight-only">
    ...
    <div class="telemetry"><div class="telemetry-item"><span>AIRSPEED</span><strong id="speed">27<small>m/s</small></strong><div class="energy-meter"><i id="energy-fill"></i></div><span id="speed-status">GLIDING</span></div><div class="telemetry-item"><span>ALTITUDE</span><strong id="altitude">43<small>m</small></strong><span class="above-water" id="lift-status">LOSING HEIGHT · FIND RISING AIR</span></div></div>
    ...
    <div class="navigation"><div class="map-title"><span>${icon('compass')} THE WORLD BEYOND</span><span>N ↑</span></div><canvas id="minimap" width="240" height="170" aria-label="Local map of nearby terrain and discovered landmarks"></canvas><div class="map-footer"><span class="live-dot"></span><span id="distance">0.00 km explored</span><span id="save-indicator">AUTOSAVE ON</span></div></div><div class="target-hint" id="target-hint"></div>
  </div>
  <div class="toast" id="toast" role="status" aria-live="polite"></div><div class="damage-flash" id="damage-flash"></div>
  <footer class="bottom-bar"><div class="control-hints"><span class="control-group"><span class="key">←</span><span class="key">→</span><span>Steer</span></span><span class="control-group"><span class="key">↑</span><span class="key">↓</span><span>Pitch / loop</span></span><span class="control-group optional-control"><span class="key wide">shift</span><span>Fan</span></span><span class="control-group"><span class="key">X</span><span>Shoot</span></span><span class="control-group optional-control"><span class="key">U</span><span>Upgrade</span></span></div><button class="how-to" id="help">How to fly ${icon('help')}</button><button class="how-to flight-only" id="save-button">Save ${icon('check')}</button></footer>
...
function damage(amount: number) {
  ...
  progress.health = Math.max(0, progress.health - amount); invulnerable = 1.1; el('damage-flash').classList.add('show');
...
function openDialog(content: string, wide = false) {
  ...
  if (mode === 'flying') { mode = 'paused'; keys.clear(); }
...
function closeDialog(resume = true) { atlasCleanup?.(); atlasCleanup = undefined; el('dialog-backdrop').hidden = true; if (resume && mode === 'paused') mode = previousMode === 'menu' ? 'menu' : 'flying'; keys.clear(); refreshMode(); lastFocus?.focus(); }
function goHome() { if (started) saveGame(); closeDialog(false); mode = 'menu'; started = false; Object.assign(state, initialFlight()); snapCamera(); refreshMode(); refreshMenu(); el('toast').classList.remove('show'); el('toast').textContent = ''; }
function pauseFlight() {
  if (mode === 'paused') { closeDialog(); return; } if (mode !== 'flying') return;
...
function showJournal(tabName = journalTab) {
  ...
  else if (tabName === 'upgrades') body = `<div class="wallet"><span>◇ ${progress.parts} scrap</span><span>✧ ${progress.cores} wind cores</span><span>${progress.fan ? 'ϟ Fan equipped' : 'Fan undiscovered'}</span></div><div class="upgrade-grid">${(Object.keys(UPGRADE_INFO) as Upgrade[]).map(kind => {
    const info = UPGRADE_INFO[kind], cost = upgradeCost(progress, kind), reason = upgradeProblem(progress, kind), level = progress.upgrades[kind];
  ...
  }).join('')}</div><p class="journal-note">Level I needs scrap. Levels II–III also need cores and a schematic: Ada studies wings; Iona studies folds; Mira knows rudders; Bram knows motors; Sela knows batteries. Wings and folds help in descending air and headwinds.</p>`;
  ...
  openDialog(`<div class="eyebrow">NOT ALL WHO WANDER ARE LOST</div><h2 id="dialog-title">Your field journal.</h2><div class="journal-tabs" role="group" aria-label="Journal section"><button data-tab="journey" class="${tabName === 'journey' ? 'active' : ''}">Atlas</button><button data-tab="quests" class="${tabName === 'quests' ? 'active' : ''}">Quests</button><button data-tab="upgrades" class="${tabName === 'upgrades' ? 'active' : ''}">Workbench</button><button data-tab="save" class="${tabName === 'save' ? 'active' : ''}">Save & backup</button></div>${body}`, true);
...
el('launch').onclick = () => launch(); el('new-game').onclick = confirmNewGame; el('sound').onclick = toggleSound; el('pause').onclick = pauseFlight; el('help').onclick = showHelp; el('settings').onclick = showSettings; el('journal').onclick = () => showJournal(); el('save-button').onclick = () => saveGame(true); el('dialog-close').onclick = () => closeDialog();
...
window.addEventListener('keydown', event => {
  ...
  if (mode === 'flying') { keys.add(event.key.length === 1 ? key : event.key); if (key === 'x') { if (!adventure.fire(state, progress) && progress.ammo <= 0) toast('Out of rubber bands. Green recharge hoops also restock ammunition.'); } if (event.key === 'Shift' && !progress.fan) toast('Find Rowan’s repair bench in Willow Mill to learn about the electric fan.'); }
});
window.addEventListener('keyup', e => keys.delete(e.key.length === 1 ? e.key.toLowerCase() : e.key));
window.addEventListener('blur', () => { keys.clear(); if (mode === 'flying') { saveGame(); pauseFlight(); } });
document.addEventListener('visibilitychange', () => { if (document.hidden && mode === 'flying') { saveGame(); pauseFlight(); } });
...
function drawMap() {
  map.clearRect(0, 0, 240, 170); const sx = (x: number) => 120 + (x - state.x) * .21, sz = (z: number) => 85 + (z - state.z) * .21;
  map.fillStyle = '#2e514666'; map.fillRect(0, 0, 240, 170);
  for (let x = 0; x < 240; x += 12) for (let z = 0; z < 170; z += 12) if (valleyFloor(state.x + (x - 120) / .21, state.z + (z - 85) / .21) > .2) { map.fillStyle = '#576e55'; map.fillRect(x, z, 12, 12); }
  [...islands, ...LAND].forEach(l => { map.fillStyle = '#718768'; map.beginPath(); map.ellipse(sx(l.x), sz(l.z), l.rx * .21, l.rz * .21, 0, 0, Math.PI * 2); map.fill(); });
  LIFTS.filter(v => Math.hypot(v.x - state.x, v.z - state.z) < 420).forEach(v => { map.strokeStyle = '#e5c887'; map.beginPath(); map.arc(sx(v.x), sz(v.z), 2.6, 0, Math.PI * 2); map.stroke(); });
  STATIONS.filter(s => progress.discovered.includes('station-' + s.id) || dist(s, state) < 230).forEach(s => { map.fillStyle = '#c1e6b9'; map.fillRect(sx(s.x) - 2, sz(s.z) - 2, 4, 4); });
  SITES.filter(s => dist(s, state) < 170).forEach(s => { map.fillStyle = '#f1d499'; map.beginPath(); map.arc(sx(s.x), sz(s.z), 2, 0, Math.PI * 2); map.fill(); });
  const goal = target(); if (goal.point) { map.fillStyle = '#e4bbf3'; map.fillRect(sx(goal.point.x) - 3, sz(goal.point.z) - 3, 6, 6); }
  adventure.enemies.forEach(e => { if (e.mesh.visible && dist(state, e.mesh.position) < 130) { map.fillStyle = '#f0907d'; map.beginPath(); map.arc(sx(e.mesh.position.x), sz(e.mesh.position.z), 1.8, 0, Math.PI * 2); map.fill(); } });
  map.save(); map.translate(120, 85); map.rotate(state.heading + (Math.cos(state.pitch) < 0 ? Math.PI : 0)); map.fillStyle = '#fff9e3'; map.beginPath(); map.moveTo(0, -5); map.lineTo(4, 5); map.lineTo(0, 2); map.lineTo(-4, 5); map.closePath(); map.fill(); map.restore();
}
function updateHUD(lift: number) {
  ...
  const encounter = nearestSite(); el('interact').hidden = !encounter; if (encounter) el('interact-name').textContent = `${encounter.kind === 'mechanism' ? 'Use' : 'Read'} ${encounter.name}`;
  el('objective-title').textContent = goal.name; el('objective-detail').textContent = goal.detail;
  el('speed').innerHTML = `${Math.round(state.speed)}<small>m/s</small>`; el('altitude').innerHTML = `${Math.round(state.y)}<small>m</small>`;
  el('energy-fill').style.width = `${Math.min(100, state.speed / 60 * 100)}%`;
  el('speed-status').textContent = state.stalled ? 'STALL · NOSE DOWN TO RECOVER' : state.speed < 15 ? 'LOW ENERGY · DIVE GENTLY' : state.speed > 40 ? 'LOOP ENERGY READY' : 'GLIDING';
  el('lift-status').textContent = lift > 2 ? `RISING AIR · +${lift.toFixed(1)} m/s` : Math.sin(state.pitch) < -.1 ? 'DIVING · GAINING SPEED' : Math.sin(state.pitch) > .1 ? 'CLIMBING · SPENDING SPEED' : 'SINKING · WATCH YOUR ALTITUDE';
  const condition = el('flight-condition'); condition.textContent = state.stalled ? 'STALL — LOWER THE NOSE' : station ? 'RECHARGING · REPAIRING · RESTOCKING' : lift > 3 ? 'RIDING AN UPDRAFT' : progress.fan && keys.has('Shift') && progress.charge > 0 ? 'FAN ASSIST' : airAt(state).name; condition.classList.toggle('stall', state.stalled);
  ...
  el('distance').textContent = `${(progress.distance / 1000).toFixed(2)} km`; if (saveTimer > 4) el('save-indicator').textContent = storageErrorShown ? 'EXPORT TO SAVE' : 'AUTOSAVE ON';
  ...
  if (goal.point) {
    ...
    el('target-hint').textContent = `${relative < -.35 ? '↶' : relative > .35 ? '↷' : '◇'} ${Math.round(dist(state, goal.point))} m · optional pin`;
...
try {
  ...
  function simulate(dt: number) {
    ...
    if (progress.fan && progress.charge <= 0 && input.boost && !fanEmptyWarned) { toast('Fan empty. Glide toward a green recharge beacon or collect a charge pickup.', 6); fanEmptyWarned = true; } if (progress.charge > 5) fanEmptyWarned = false;
  ...
  function animate(now: number) {
    ...
    if (mode === 'flying') { accumulator += dt; while (accumulator >= 1 / 60) { simulate(1 / 60); accumulator -= 1 / 60; } } else { accumulator = 0; adventure.update(mode === 'paused' ? 0 : dt, time, state, progress, false, () => {}, () => {}); }
    ...
    world.update(time, position, state.heading, mode === 'flying' ? state.speed : 6); renderer.render(world.scene, camera);
    if (windGain && audioContext) windGain.gain.setTargetAtTime(soundEnabled && mode !== 'paused' ? .06 + (mode === 'flying' ? state.speed * .002 : 0) : 0, audioContext.currentTime, .3);
