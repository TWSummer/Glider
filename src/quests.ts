...
const LEADS: Record<string, [string, string, string]> = {
  ...
  'sigil-copper': ['Copper sigil', 'Coppervein · far northern ore chamber, just before the broken railway.', 'mine'],
...
export function questSteps(p: Progress, q: Quest): QuestStep[] {
  return q.goals.flatMap((g): QuestStep[] => {
    ...
    if (g.kind === 'relic') return [{ id: 'skyheart', title: 'Recover the Skyheart', hint: sanctuaryOpen(p) ? 'Skyheart Temple · ground-floor inner sanctuary. Fly through the golden armillary.' : 'The temple sanctuary opens with all three valley sigils and both awakened altars.', region: 'temple', done: p.relic }];
